...
//$instruccion = "SELECT p.idproyecto, p.nombreproyecto, i.nombreinstitucion, p.estadoproyecto FROM proyecto p, institucion i WHERE p.idinstitucion=i.idinstitucion and p.idproyecto NOT IN(SELECT pr.idproyecto FROM proyecto pr, solicitud_proyecto sp,alumno_proyecto ap WHERE pr.idproyecto=sp.idproyecto AND pr.idproyecto=ap.idproyecto)";
...
$instruccion = "SELECT p.idproyecto, pp.nomproyecto, pp.nominstitucion FROM proyecto p, propuesta_proyecto pp, se_convierte sc, propuesta_carrera pc WHERE p.idproyecto=sc.idproyecto AND sc.idpropuesta=pp.idpropuesta AND pc.idpropuesta=pp.idpropuesta AND (p.estadoProyecto='D' OR p.estadoProyecto='A') and idcarrera=$idcarrera";

$consulta_proyectos = pg_query($instruccion) or die ("<SPAN CLASS='error'>Fallo en consulta de proyectos disponibles!!</SPAN>".pg_last_error());
?>
...
<!------------------------------------------------------------------------------------------------------------------------>
		<tr>
            ...
            <p> Selecciona un proyecto para ver los detalles y poder aplicar </p>
			</td>
		</tr>
<!------------------------------------------------------------------------------------------------------------------------>
		<tr>
			<td>
			<?php					
			$cantidad_proyectos = pg_query("SELECT count(idproyecto) FROM proyecto p WHERE p.estadoproyecto='D'") or die ("<SPAN CLASS='error'>Fallo la consulta cantiproy!!</SPAN>".pg_last_error());
			...
			<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
				<tr>
					...
					<table cellpadding="0" cellspacing="0" id="table" class="tinytable">
						<thead class="titulo1">
					<tr>
						<th onMouseOver="toolTip('Ordenar por codigo',this)" width="70"><h3>Codigo</h3></th>
						<th onMouseOver="toolTip('Ordenar por proyecto',this)" width="200"><h3>Proyecto</h3></th>
						<th onMouseOver="toolTip('Ordenar por institucion',this)" width="100"><h3>Institucion</h3></th>
						...
						<tbody class="subtitulo2">
							...
							?>
					<tr align="center">
											<td><?php echo "<a title='Ver Proyecto' style='color: black;' href='frmVerProyecto.php?id=$proyectos[0]'>".$proyectos[0]."</a>";?></td>
											<td><?php echo "<a title='Ver Proyecto' style='color: black;' href='frmVerProyecto.php?id=$proyectos[0]'>".$proyectos[1]."</a>";?></td>
											<td><?php echo "<a title='Ver Proyecto' style='color: black;' href='frmVerProyecto.php?id=$proyectos[0]'>".$proyectos[2]."</a>";?></td>
				...
				<tr>
					...
					<td>
						...
						<img src="../../imagenes/icono_recargar.png" align="top" onClick="redireccionar('frmConsultarDisponible.php');" onMouseOver="toolTip('Actualizar',this)" class="manita">
