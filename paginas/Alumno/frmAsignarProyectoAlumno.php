...
// $instruccion = "select a.idalumno,a.nombreAlumno||' '||a.apellidosalumno as alumno,a.carnet from alumno as a where a.idalumno NOT IN (SELECT sp.idalumno FROM solicitud_proyecto as sp ) AND a.idalumno NOT IN (SELECT ap.idalumno FROM alumno_proyecto as ap )";

//NUEVA
$instruccion="select a.idalumno,a.nombreAlumno||' '||a.apellidosalumno as alumno,a.carnet from alumno as a where a.idalumno NOT IN (SELECT sp.idalumno FROM solicitud_proyecto as sp WHERE estado='P' ) AND a.idalumno NOT IN (SELECT ap.idalumno FROM alumno_proyecto as ap WHERE estadoAlumnoProyecto='P' OR estadoAlumnoProyecto='L' ) AND a.idalumno NOT IN (SELECT h.idalumno FROM propuesta_proyecto pp INNER JOIN hace h ON h.idPropuesta=pp.idPropuesta WHERE pp.estadoProp='P') and idalumno>0";
...
<!------------------------------------------------------------------------------------------------------------------------>
		<tr>
			<td align="center">
            <h2>SELECCIONE ALUMNO PARA ASIGNARLE UN PROYECTO</h2>
			</td>
		</tr>
<!------------------------------------------------------------------------------------------------------------------------>
		<tr>
			...
			$cantidad = pg_num_rows($consulta_alumnos);
			if($cantidad <> 0){
			?>
			...
			<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
				<tr>
					...
					<tr>
						...
						<th onMouseOver="toolTip('Ordenar por carnet',this)" width="80"><h3>Nombre Alumno</h3></th>
						</tr>
						</thead>
						<tbody class="subtitulo2">
							...
							?>
					<tr align="center">
											<td><?php echo "<a title='Ver Proyecto' style='color: black;' href='../Alumno/frmConsultarProyecto.php?id=$alumnos[0]&nom=$alumnos[1]'>".$alumnos[2]."</a>";?></td>
											<td><?php echo "<a title='Ver Proyecto' style='color: black;' href='../Alumno/frmConsultarProyecto.php?id=$alumnos[0]&nom=$alumnos[1]'>".$alumnos[1]."</a>";?></td>
						  </tr>
						<?php
						}
						?>
				...
				<tr>
					...
					<td>
						   ...
						   <img src="../../imagenes/icono_recargar.png" align="top" onClick="redireccionar('frmAsignarProyectoALumno.php');" onMouseOver="toolTip('Actualizar',this)" class="manita">
				...
				<table align="center" class="alerta error centro">
					<tr>
						<td align="center" colspan="3">
							No hay alumnos sin proyecto asignado en el registro del sistema.</td>
					</tr>
				</table>
				<?php } ?>
				<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
