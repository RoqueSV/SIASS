...
function Header()
{
	...
	$this->Ln(20);
	$this->Cell(80);
   	$this->Cell(30,10,'LISTADO DE ALUMNOS QUE ABRIERON EXPEDIENTE DE SU SERVICIO SOCIAL.',0,1,'C');
	//SALTO DE LINEA
	$this->Ln(5);
}
...
where a.porcentaje >= c.porcentajecarrera and idalumno >0 order by codcarrera, carnet)
EXCEPT
(select carnet, apellidosalumno|| ', '||nombrealumno nombre, porcentaje, codcarrera  
from alumno a join carrera c on  a.idcarrera=c.idcarrera
where a.porcentaje >= c.porcentajecarrera and a.idalumno in (select idalumno from alumno_proyecto)
